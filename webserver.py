...
def handle_request(request_data):
    ...
    if type(state) is EventState:
        if state == EventState.EVENT_CREATED:
            ...
            response.message(
                "Would you like this event to be: \n1) Private \n2) Friends of Friends \n3) Public")
            return str(response)
            ...
            response.message(
                "Send us the phone number of or contact of someone you want to invite")
            return str(response)
            ...
            for i in range(int(NumMedia)):
                contacts_list.append(handle_vcf_url(
                    request_data['MediaUrl' + str(i)]))
            if(len(body) == 10):
        ...
        if state == AttendeeState.INVITE_SENT or state == AttendeeState.INVITE_MAYBE:
            if body.lower() == 'yes':
                if not isFull(key):
                    ...
                    response.message(
                        "Thank you for accepting. What's your name?")
                else:
                    response.message(
                        "I'm sorry, the event is full. You will be notified if any openings occur.")
                    inviteMaybe(phone_number)
                return str(response)
            elif body.lower() == 'no':
                inviteDeclined(phone_number)
                response.message(
                    "Sorry that you can't make it. Thank you for using Events Everywhere")
                return str(response)
            elif body.lower() == 'maybe':
                inviteMaybe(phone_number)
                response.message(
                    "Please respond yes or no when you know if you'll be able to make it")
                return str(response)
            else:
                response.message(
                    "Sorry we were unable to understand what you said. Please send yes, no, or maybe")
                return str(response)
            ...
            if canAdd(phone_number, AttendeeState.ATTENDEE_NAMED):
                response.message(
                    "If you'd like please send us the name or contact of someone else you'd like to invite, then type DONE")
            else:
        ...
        if body.upper() == 'START':
            if createEvent(phone_number):
                response.message(
                    "Welcome! Thank you for planning your event with Events Everywhere. Please provide a name for your event.")
                return str(response)
            else:
                response.message(
                    "Please finish editing your other event first")
                return str(response)
        if body == 'HELP':
...
if __name__ == '__main__':
    ...
    scheduler.add_job(remind_attendees, 'interval', hours = 5)
    scheduler.start()
