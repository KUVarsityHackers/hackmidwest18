...
def setCreatorNameEvent(creator, name):
    ...
    try:
        ...
        c.execute('INSERT INTO attendees (name, phone, eventID, status, isOwner, canAdd) VALUES (?, ?, ?, ?, ?, ?)', [
                  str(name), creator, int(eventID), int(AttendeeState.DONE_PROVIDED), True, True])
        closeConnection(conn)
        return True
...
def sendInvite(sender, invitee):
    ...
    try:
        ...
        c.execute('UPDATE events SET status = ? WHERE key = ?',
                  [EventState.ATTENDEES_ADDED, eventID])
        print("UPDATED")
        c.execute('SELECT visibility FROM events WHERE key = ?', [eventID])
...
def getPersonName(phone_number):
    ...
    c.execute('SELECT name FROM attendees WHERE phone = ?', [phone_number])
...
def getEventDescription(key):
    ...
    c.execute('SELECT details FROM events WHERE key = ?', [key])
    creatorNames = c.fetchone()
    if creatorNames is not None:
        for creatorName in creatorNames:
            if len(creatorName) > 0:
...
def getEventTime(key):
    ...
    c.execute('SELECT date FROM events WHERE key = ?', [key])
    creatorNames = c.fetchone()
    if creatorNames is not None:
        for creatorName in creatorNames:
            if len(creatorName) > 0:
...
def getOpenEventName(key):
    ...
    c.execute('SELECT title FROM events WHERE key = ?', [key])
...
def hadUnfinishedEvent(creator):
    ...
    c.execute('SELECT status FROM events WHERE creator = ?', [creator])
...
def setAttendeeName(phone, name):
    ...
    try:
        c.execute('UPDATE attendees set name = ?, status = ? where phone = ?', [
                  name, int(AttendeeState.ATTENDEE_NAMED), phone])
        closeConnection(conn)
        return True
    except:
...
def getState(phone):
    ...
    c.execute('SELECT status FROM events WHERE creator = ?', [phone])
    ...
    c.execute('SELECT status FROM attendees WHERE phone = ?', [phone])
...
def setAttendeeDone(phone_number):
    ...
    try:
        c.execute('''UPDATE attendees SET status = ? WHERE phone = ? AND status = ?''', [
                  AttendeeState.DONE_PROVIDED, phone_number, AttendeeState.ATTENDEE_NAMED])
        closeConnection(conn)
        return True
    except:
...
def isFull(key):
    ...
    c = conn.cursor()
    print("Key %s" % key)
    c.execute('SELECT status FROM attendees WHERE eventID = ?', [key])
    statuses = c.fetchall()
    c.execute('SELECT cap FROM events WHERE key = ?', [key])
    cap = c.fetchone()[0]
    closeConnection(conn)
    attending = 0
